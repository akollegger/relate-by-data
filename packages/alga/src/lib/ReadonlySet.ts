...
import { Order as Ord } from '@effect/data/typeclass/Order';
import { not, Predicate, Refinement } from '@effect/data/Predicate';
...
export function partition<A, B extends A>(
  refinement: Refinement<A, B>
): (set: ReadonlySet<A>) => [ReadonlySet<A>, ReadonlySet<B>];
export function partition<A>(
  predicate: Predicate<A>
): <B extends A>(set: ReadonlySet<B>) => [ReadonlySet<B>, ReadonlySet<B>];
export function partition<A>(
  predicate: Predicate<A>
): (set: ReadonlySet<A>) => [ReadonlySet<A>, ReadonlySet<A>];
export function partition<A>(
  predicate: Predicate<A>
): (set: ReadonlySet<A>) => [ReadonlySet<A>, ReadonlySet<A>] {
  return (set: ReadonlySet<A>) => {
...
export function union<A>(
  ...
  me: ReadonlySet<A>,
  that?: ReadonlySet<A>
) => ReadonlySet<A> | ((me: ReadonlySet<A>) => ReadonlySet<A>) {
  const elemE = elem(E);
  return (me, that?) => {
    if (that === undefined) {
...
export function intersection<A>(
  ...
  me: ReadonlySet<A>,
  that?: ReadonlySet<A>
) => ReadonlySet<A> | ((that: ReadonlySet<A>) => ReadonlySet<A>) {
  const elemE = elem(E);
  return (me, that?) => {
    if (that === undefined) {
...
export function partitionMap<B, C>(
  ...
  f: (a: A) => Either<B, C>
) => (set: ReadonlySet<A>) => [ReadonlySet<B>, ReadonlySet<C>] {
...
export function foldMap<A, M>(
  ...
  const toReadonlyArrayO = toReadonlyArray(O);
  return (f) => (fa) =>
    toReadonlyArrayO(fa).reduce((b, a) => M.combine(b, f(a)), M.empty);
}
...
export const reduceRight = <A>(
  ...
  const toReadonlyArrayO = toReadonlyArray(O);
  return (b, f) => (fa) =>
    toReadonlyArrayO(fa).reduceRight((b, a) => f(a, b), b);
};
...
export function insert<A>(
  E: Eq<A>
): (a: A) => (set: ReadonlySet<A>) => ReadonlySet<A> {
  const elemE = elem(E);
  return (a) => (set) => {
    if (!elemE(a)(set)) {
      const r = new Set(set);
...
export const compact = <A>(
  E: Eq<A>
): ((fa: ReadonlySet<Option<A>>) => ReadonlySet<A>) => filterMap(E)(identity);
...
export function separate<E, A>(
  EE: Eq<E>,
  EA: Eq<A>
): (fa: ReadonlySet<Either<E, A>>) => [ReadonlySet<E>, ReadonlySet<A>] {
  return (fa) => {
...
export function every<A, B extends A>(
  refinement: Refinement<A, B>
): Refinement<ReadonlySet<A>, ReadonlySet<B>>;
...
export function isSubset<A>(
  ...
  me: ReadonlySet<A>,
  that?: ReadonlySet<A>
) => boolean | ((me: ReadonlySet<A>) => boolean) {
  const elemE = elem(E);
  return (me, that?) => {
    if (that === undefined) {
...
export function elem<A>(
  ...
  a: A,
  set?: ReadonlySet<A>
) => boolean | ((set: ReadonlySet<A>) => boolean) {
  return (a, set?) => {
    if (set === undefined) {
      const elemE = elem(E);
...
export const getUnionSemigroup = <A>(
  isEquivalent: Eq<A>
): Semigroup<ReadonlySet<A>> => semigroup.make(union(isEquivalent)) as any;

/**
 * @category instances
 * @since 2.5.0
 */
// export const getUnionMonoidOriginal = <A>(E: Eq<A>): Monoid<ReadonlySet<A>> => ({
 ...
 * @since 2.5.0
 */
// export const getIntersectionSemigroupOriginal = <A>(E: Eq<A>): Semigroup<ReadonlySet<A>> => ({
//   concat: intersection(E)
// })

export const getIntersectionSemigroup = <A>(
  isEquivalent: (self: A, that: A) => boolean
): Semigroup<ReadonlySet<A>> =>
  semigroup.make(intersection(isEquivalent)) as any;
...
export const fromArray: <A>(
  E: Eq<A>
) => (as: ReadonlyArray<A>) => ReadonlySet<A> = fromReadonlyArray;
