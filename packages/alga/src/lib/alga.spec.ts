...
const something = fc
  .anything()
  .filter((a) => a != null && (typeof a !== 'number' || !Number.isNaN(a)));

describe('Algebraic graphs suite', () => {
  const eqAnything = fromEquals(strictEqual);
  const ordAnything = fromCompare<any>((x, y) =>
    x === y ? 0 : x < y ? -1 : 1
  ...
  describe('Laws', () => {
    it('Overlay is commutative', () =>
      fc.assert(
        fc.property(something, something, (a, b) => {
    ...
    it('Overlay is associative', () =>
      fc.assert(
        fc.property(something, something, something, (a, b, c) => {
    ...
    it('Connect distributes over Overlay', () =>
      fc.assert(
        fc.property(something, something, something, (a, b, c) => {
          fc.pre(new Set([a, b, c]).size === 3);
          ...
          const a_times_b_plus_a_times_c = G.overlay(
            G.connect(va, vb),
            G.connect(va, vc)
          );
          const a_plus_b_times_c = G.connect(G.overlay(va, vb), vc);
          const a_times_c_plus_b_times_c = G.overlay(
            G.connect(va, vc),
            G.connect(vb, vc)
          );

          expect(
            G.eqGraph.equals(a_times_b_plus_c, a_times_b_plus_a_times_c)
          ).toBeTruthy();
          expect(
            G.eqGraph.equals(a_plus_b_times_c, a_times_c_plus_b_times_c)
          ).toBeTruthy();
        })
      ));
    ...
    it('Decomposition', () =>
      fc.assert(
        fc.property(something, something, something, (a, b, c) => {
          ...
          const a_times_b_plus_a_times_c_plus_b_times_c = G.overlay(
            G.overlay(G.connect(va, vb), G.connect(va, vc)),
            G.connect(vb, vc)
          );
          ...
          expect(
            G.eqGraph.equals(
              a_times_b_times_c,
              a_times_b_plus_a_times_c_plus_b_times_c
            )
          ).toBeTruthy();
        })
      ));
      ...
      fc.assert(
        fc.property(
          ...
          (a, b, c, d) => {
            ...
            const g = G.connect(
              G.edge(args[0], args[1]),
              G.edge(args[2], args[3])
            );
            const al = G.toAdjacencyList(ordAnything)(g);

            expect(new Set(al.map(fst))).toEqual(new Set(args));
            expect(new Set(al.map(snd).map((el) => el.length))).toEqual(
              new Set([0, 1, 2, 3])
            );
          }
        )
