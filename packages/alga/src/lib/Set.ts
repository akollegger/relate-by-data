...
import { Equivalence as Eq } from '@effect/data/typeclass/Equivalence';
...
export function partition<A, B extends A>(
  refinement: Refinement<A, B>
): (set: Set<A>) => [Set<A>, Set<B>];
export function partition<A>(
  predicate: Predicate<A>
): <B extends A>(set: Set<B>) => [Set<B>, Set<B>];
export function partition<A>(
  predicate: Predicate<A>
...
export function union<A>(
  E: Eq<A>
): (me: Set<A>, that?: Set<A>) => Set<A> | ((me: Set<A>) => Set<A>) {
  const elemE = elem(E);
  return (me, that?) => {
    if (that === undefined) {
...
export function intersection<A>(
  E: Eq<A>
): (me: Set<A>, that?: Set<A>) => Set<A> | ((that: Set<A>) => Set<A>) {
  const elemE = elem(E);
  return (me, that?) => {
    if (that === undefined) {
...
export function partitionMap<B, C>(
  EB: Eq<B>,
  EC: Eq<C>
): <A>(f: (a: A) => Either<B, C>) => (set: Set<A>) => [Set<B>, Set<C>] {
 ...
 * @since 2.11.0
 */
// export const getUnionSemigroupOriginal = <A>(E: Eq<A>): Semigroup<Set<A>> => ({
//   concat: union(E)
// })

export const getUnionSemigroup = <A>(isEquivalent: Eq<A>): Semigroup<Set<A>> =>
  semigroup.make(union(isEquivalent)) as any;

/**
 * @category instances
 * @since 2.0.0
 */
// export const getUnionMonoidOriginal = <A>(E: Eq<A>): Monoid<Set<A>> => ({
...
export const getIntersectionSemigroup = <A>(
  isEquivalent: (self: A, that: A) => boolean
): Semigroup<Set<A>> => semigroup.make(intersection(isEquivalent)) as any;
...
export const reduceRight: <A>(
  O: Ord<A>
) => <B>(b: B, f: (a: A, b: B) => B) => (fa: Set<A>) => B = RS.reduceRight;
...
export const isSubset: <A>(
  E: Eq<A>
) => (that: Set<A>) => (me: Set<A>) => boolean = RS.isSubset;
